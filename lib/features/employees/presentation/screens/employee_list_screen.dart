...
class EmployeeListScreen extends ConsumerWidget {
              ...
              PermissionWrapper(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text('₹${employee.salary}', style: context.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600, color: context.tokens.success)),
                  ],
                ),
              ),
              Row(
                children: [
                  IconButton(
                    icon: Icon(Icons.check_circle_outline, color: isPresent ? context.tokens.success : context.theme.disabledColor),
                    onPressed: () => ref.read(employeeNotifierProvider.notifier).updateAttendance(employee.id, 'Present'),
                  ),
                  IconButton(
                    icon: Icon(Icons.cancel_outlined, color: !isPresent ? context.colorScheme.error : context.theme.disabledColor),
  ...
  Widget _buildAttendanceBadge(BuildContext context, String status) {
    final color = status == 'Present' ? context.tokens.success : context.colorScheme.error;
    return Container(
